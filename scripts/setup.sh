...
print_header() {
    ...
    cat << "EOF"
    ╔════════════════════════════════════════════════════════╗
    ║         PiPassive - Setup Configuration                ║
    ║    Guided Configuration for Passive Income Services    ║
    ╚════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}"
}
...
check_env_file() {
    if [[ -f "$ENV_FILE" ]]; then
        ...
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            log_info "I will keep existing configurations and only update new values."
            return 1
        fi
    fi
    return 0
...
test_connectivity() {
    ...
    if timeout 5 curl -sf "$test_url" >/dev/null 2>&1; then
        ...
        log_warning "$service_name not reachable (might be normal)"
        return 1
    fi
}

# Ask input with default value
ask_input() {
    local prompt=$1
    local var_name=$2
    ...
    # If value exists, use it as default
    if [[ -n "$existing_value" ]]; then
        default_value="$existing_value"
    fi

    # Print prompt with colors (not captured in ask_input)
    if [[ "$is_password" == "true" ]]; then
        {
        echo -ne "${CYAN}${prompt}${NC}"
        if [[ -n "$default_value" ]]; then
            echo -ne " ${YELLOW}[current: ****]${NC}"
        fi
        echo -ne ": "
        } >&2
        read -rs value
    ...
    # Print only the value (without colors)
    echo "$value"
}
...
configure_service() {
    ...
    read -p "Do you want to configure ${service_name}? (y/n) " -n 1 -r response
    echo
    if [[ ! $response =~ ^[Yy]$ ]]; then
        log_info "Skipped ${service_name}"
        return 1
    fi
    return 0
}
...
main() {
    ...
    log_info "Guided Configuration Setup"
    echo
    log_info "This script will guide you through the configuration of all services."
    log_info "Press ENTER to use the default value [in brackets]"
    echo
    ...
    # Interactive installation of system services
    echo
    echo -e "${MAGENTA}═══════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}System Services Installation (Optional)${NC}"
    echo -e "${MAGENTA}═══════════════════════════════════════════════════════${NC}"
    echo
    ...
    echo -e "  1. Start services: ${GREEN}./manage.sh start${NC}"
    echo -e "  2. Access: ${CYAN}http://pipassive.local${NC}"
    echo -e "  3. Web configuration: ${CYAN}http://pipassive.local/setup${NC}"
    echo
    echo -e "${CYAN}Available services:${NC}"
    echo -e "  • 8 Docker services (Honeygain, Pawns, PacketStream, Repocket, EarnFM, MystNode, PacketShare, TraffMonetizer)"
    if command -v earnapp >/dev/null 2>&1; then
        echo -e "  • ${GREEN}✓${NC} EarnApp (system service - installed)"
    else
        echo -e "  • ○ EarnApp (optional - not installed)"
    fi
    echo
    echo -e "${YELLOW}⚠️  IMPORTANT - READ:${NC}"
    echo -e "  🔴 ${RED}MystNode requires manual configuration!${NC}"
    echo -e "     After 'manage.sh start', go to: ${CYAN}http://pipassive.local:4449${NC}"
    echo -e "     and complete the setup procedure on the dashboard."
    echo
    log_warning "Never share the .env file!"
    echo
}
